            ...
            <tbody class="bg-white">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 text-gray-500">
                            <a href="{{route('candidato.index', ['id' => $vacante->id])}}"
                                class="text-gray-500 hover:text-gray-600">
                                {{$vacante->candidatos->count()}} Candidatos
                            </a>
                        </td>
                        <td class="inline-flex px-6 py-4 whitespace-no-wrap border-gray-200 text-sm leading-5 font-medium">
                                <a href="{{route('vacantes.edit', ['vacante' => $vacante->id])}}"
                                class="flex-1 text-teal-600 hover:text-teal-900 mr-5">Editar</a>
                                <eliminar-vacante
                                    vacante-id="{{$vacante->id}}">
                                </eliminar-vacante>
                                <a href="{{route('vacantes.show', ['vacante' => $vacante->id])}}"
                                    class="flex-1 text-blue-600 hover:text-blue-900">Ver
                                </a>
                        </td>
                    </tr>
                    @endforeach
